...
alias use.skill { 
  ...
  if ($3 = deathstrike) { $skill.deathstrike($1) }
...
alias skill.speedup { $set_chr_name($1)
  ...
  if ($readini($char($1), info, flag) = $null) { 
    ...
    inc %spd %hp.cost
    var %hp.cost2 $round($calc(%spd / 3),0)
    set %spd-inc %hp.cost2
    if (%hp.cost >= $readini($char($1), battle, hp)) { $display.message($readini(translation.dat, errors, NotEnoughHPForSkill) ,private) | halt }
    else {  writeini $char($1) battle hp $calc($readini($char($1), battle, hp) - %hp.cost) | writeini $char($1) Battle spd %hp.cost2  }
  }
...
; DEATH STRIKE
;=================
on 3:TEXT:!death strike*:*: { $skill.deathstrike($nick) }
on 3:TEXT:!deathstrike*:*: { $skill.deathstrike($nick) }

alias skill.deathstrike { $set_chr_name($1)
  if ($person_in_mech($1) = true) { $display.message($readini(translation.dat, errors, Can'tDoThatInMech), private) | halt }
  $no.turn.check($1)
  if (no-skill isin %battleconditions) { $display.message($readini(translation.dat, battle, NotAllowedBattleCondition),private) | halt }
  $amnesia.check($1, skill) 
  ...
  $checkchar($1)
  if ($skillhave.check($1, DeathStrike) = false) { $set_chr_name($1) | $display.message($readini(translation.dat, errors, DoNotHaveSkill), private)  | halt }
  if (%battleis = off) { $display.message(04There is no battle currently!, private) | halt }

  $check_for_battle($1)

  var %skill.name DeathStrike
  if (($skill.needtoequip(%skill.name) = true) && ($skill.equipped.check($1, %skill.name) = false)) { $display.message($readini(translation.dat, errors, SkillNeedsToBeEquippedToUse), private) | halt } 


  ; Check to see if enough time has elapsed
  $skill.turncheck($1, DeathStrike, !DeathStrike strike, true)

  ; Decrease the action points
  $action.points($1, remove, $skill.actionpointcheck(deathstrike))

  ; Display the desc. 
  if ($readini($char($1), descriptions, DeathStrike) = $null) { set %skill.description forces energy into $gender($1) weapon, causing the next blow done with it to be quadro power (4Death Strike) }
  else { set %skill.description $readini($char($1), descriptions, DeathStrike) }
  $set_chr_name($1) | $display.message(12 $+ %real.name  $+ %skill.description, battle) 

  ; Toggle the flag & write the last used time.
  writeini $char($1) skills deathstrike.on on
  writeini $char($1) skills deathstrike.time %true.turn

  writeini $txtfile(battle2.txt) style $1 $+ .lastaction deathstrike

  $skill.nextturn.check(DeathStrike, $1)
}
...
on *:TEXT:!Tameing *:#: { set %mon.name2 $2 | $skill.taming($nick)  }

alias skill.taming { $set_chr_name($1)
  if ($person_in_mech($1) = true) { $display.message($readini(translation.dat, errors, Can'tDoThatInMech), private) | halt }
  $no.turn.check($1)
  if (no-skill isin %battleconditions) { $display.message($readini(translation.dat, battle, NotAllowedBattleCondition),private) | halt }
  $amnesia.check($1, skill) 
  ...
  ;check if taming already used in the battle
  ;if ($readini($char($1), Skills, taming.on) == on) { msg $chan This Skill cannot use again in this battle | /next }
  ...
  if ((%check.item = $null) || (%check.item <= 0)) { $set_chr_name($1) | $display.message(04Error: %real.name does not have a EmptyPokeBall to perform this skill, private) | halt }
  if (%check.item >= 1) && ($readini($char($1), Taming_Monster, Name)) { $set_chr_name($1) | $display.message(04Error: %real.name You have also a PoweredPokeball with $readini($char($1), Taming_Monster, Name) inside you must clear the data for a new pokemon , private) | halt

    ; Decrease the action points
    $action.points($1, remove, $skill.actionpointcheck(taming))

    set %targethp2 $readini($char(%mon.name2), BaseStats, HP)
    set %percent2 $calc(%targethp2 * 30 / 100)
    set %newhp2 $round(%percent2,0)
    set %targetbattlehp2 $readini($char(%mon.name2), Battle, HP)
    set %catchchance2 $rand(1, 100)
    set %skill.description try to catch %mon.name into a Pokebal but....
    $display.message(12 $+ %real.name  $+ %skill.description, battle) 
    /msg $chan catchchance %catchchance2
    if (%catchchance2 <= 30) {
      if (%targetbattlehp2 <= %newhp2) { 
        ; Display the desc. 
        if ($readini($char($1), descriptions, taming) = $null) { set %skill.description Successful catch %mon.name2 into a Powered Pokeball. }
        else { set %skill.description $readini($char($1), descriptions, Taming) }
        $set_chr_name($1) 
        $display.message(12 $+ %real.name  $+ %skill.description, battle) 
        writeini $char($1) Taming_Monster Name $readini($char(%mon.name2), BaseStats, Name)
        writeini $char($1) Taming_Monster HP $readini($char(%mon.name2), BaseStats, HP)
        writeini $char($1) Taming_Monster TP $readini($char(%mon.name2), BaseStats, TP)
        writeini $char($1) Taming_Monster str $readini($char(%mon.name2), BaseStats, str)
        writeini $char($1) Taming_Monster def $readini($char(%mon.name2), BaseStats, def)
        writeini $char($1) Taming_Monster int $readini($char(%mon.name2), BaseStats, int)
        writeini $char($1) Taming_Monster spd $readini($char(%mon.name2), BaseStats, spd)
        writeini $char($1) Taming_Monster IgnitionGauge $readini($char(%mon.name2), BaseStats, IgnitionGauge)
        writeini $char($1) Taming_Monster Level $get.level(%mon.name2)
        ;Set The Other Monster as Dead
        writeini $char(%mon.name2) battle status dead
        writeini $char(%mon.name2) battle hp 0
        $decrease_item($1, EmptyPokeball)
        writeini $char($1) Item_Amount PoweredPokeball 1 
      }
    }
    if (%catchchance2 >= 30) { msg $chan failed to catch %mon.name2 }
    ; Toggle the utsusemi-on flag & write the last used time.
    writeini $char($1) skills taming.on on

    writeini $txtfile(battle2.txt) style $1 $+ .lastaction taming

    $skill.nextturn.check(taming, $1)
  }
}
;=================
; FULL BRING SKILL
...
alias skill.bloodboost { $set_chr_name($1)
  ...
  inc %str $skill.bloodboost.calculate($1)
...
alias skill.bloodboost.calculate {
  ...
  return %percent.increase
  var %staterhoehung %percent.increase
}
...
alias skill.ThrowPoke { $set_chr_name($1)
  if ($person_in_mech($1) = true) { $display.message($readini(translation.dat, errors, Can'tDoThatInMech), private) | halt }
  if (%battle.type = dungeon) { $display.message($readini(translation.dat, errors, Can'tUseThisSkillInDungeon), private) | halt }
  if (%battle.type = dragonhunt) { $display.message($readini(translation.dat, errors, Can'tUseThisSkillInDragonHunt), private) | halt }
  $no.turn.check($1)
  ...
  if (%battleis = off) { $display.message($readini(translation.dat, errors, NoBattleCurrently),private) | halt }
  $check_for_battle($1)
  var %pokename $replace($readini($char($1), Taming_Monster, Name), $chr(32),_)
  if (($readini($char($1), info, flag) = $null) && ($readini($char($1 $+ _summon), battle, hp) != $null)) { $display.message($readini(translation.dat, errors, CanOnlyUseSummonOrDoppel), private) | halt }
  if ($isfile($char(%pokename $+ $1 $+ _clone)) = $true) { $set_chr_name($1) | $display.message(04Error: %real.name has already used this skill for this battle and cannot use it again!, private) | halt }

  ; Decrease the action points
  $action.points($1, remove, $skill.actionpointcheck(throwpoke))

  ; Display the desc. 
  var %pokeball $readini($char($1), Item_Amount, PoweredPokeball)
  if (%pokeball == $null) { msg $chan No Powered Pokeball in your Inventory | /next }
  if ($readini($char($1), descriptions, throwpoke) = $null) { set %skill.description Unleashes the power of a powerful Pokeballs to summon a $readini($char($1), Taming_Monster, Name) it will helps you in the battle. }
  else { set %skill.description $readini($char($1), descriptions, throwpoke) }
  $set_chr_name($1) | $display.message(12 $+ %real.name  $+ %skill.description, battle) 
  ...
  .copy $mon(%pokename) $char(%pokename $+ $1 $+ _clone)
  if ($readini($char(%pokename $+ $1 $+ _clone), info, flag) = monster) { writeini $char(%pokename $+ $1 $+ _clone) info flag npc | writeini $char(%pokename $+ $1 $+ _clone) monster type pokemon }

  writeini $char(%pokename $+ $1 $+ _clone) BaseStats HP $readini($char($1), Taming_Monster, HP)
  writeini $char(%pokename $+ $1 $+ _clone) BaseStats TP $readini($char($1), Taming_Monster, TP)
  writeini $char(%pokename $+ $1 $+ _clone) BaseStats Str $readini($char($1), Taming_Monster, spd)
  writeini $char(%pokename $+ $1 $+ _clone) BaseStats def $readini($char($1), Taming_Monster, def)
  writeini $char(%pokename $+ $1 $+ _clone) BaseStats int $readini($char($1), Taming_Monster, int)
  writeini $char(%pokename $+ $1 $+ _clone) BaseStats spd $readini($char($1), Taming_Monster, spd)
  writeini $char(%pokename $+ $1 $+ _clone) BaseStats IgnitionGauge $readini($char($1), Taming_Monster, IgnitionGauge )

  writeini $char(%pokename $+ $1 $+ _clone) Battle HP $readini($char($1), Taming_Monster, HP)
  writeini $char(%pokename $+ $1 $+ _clone) Battle TP $readini($char($1), Taming_Monster, TP)
  writeini $char(%pokename $+ $1 $+ _clone) Battle Str $readini($char($1), Taming_Monster, spd)
  writeini $char(%pokename $+ $1 $+ _clone) Battle def $readini($char($1), Taming_Monster, def)
  writeini $char(%pokename $+ $1 $+ _clone) Battle  int $readini($char($1), Taming_Monster, int)
  writeini $char(%pokename $+ $1 $+ _clone) Battle spd $readini($char($1), Taming_Monster, spd)
  writeini $char(%pokename $+ $1 $+ _clone) Battle IgnitionGauge $readini($char($1), Taming_Monster, IgnitionGauge )

  /timersetdata1 1 1 writeini $char(%pokename $+ $1 $+ _clone) Battle HP $readini($char($1), Taming_Monster, HP)
  /timersetdata1 1 1 writeini $char(%pokename $+ $1 $+ _clone) Battle TP $readini($char($1), Taming_Monster, TP)
  /timersetdata1 1 1 writeini $char(%pokename $+ $1 $+ _clone) Battle Str $readini($char($1), Taming_Monster, spd)
  /timersetdata1 1 1 writeini $char(%pokename $+ $1 $+ _clone) Battle def $readini($char($1), Taming_Monster, def)
  /timersetdata1 1 1 writeini $char(%pokename $+ $1 $+ _clone) Battle int $readini($char($1), Taming_Monster, int)
  /timersetdata1 1 1 writeini $char(%pokename $+ $1 $+ _clone) Battle spd $readini($char($1), Taming_Monster, spd)
  var %monster.level $readini($char($1), Taming_Monster, Level)
  writeini $char(%pokename $+ $1 $+ _clone) Battle IgnitionGauge $readini($char($1), Taming_Monster, IgnitionGauge )
  $levelsync(%pokename $+ $1 $+ _clone, %monster.level)
  set %curbat $readini($txtfile(battle2.txt), Battle, List)
  %curbat = $addtok(%curbat,%pokename $+ $1 $+ _clone,46)
  writeini $txtfile(battle2.txt) Battle List %curbat
  write $txtfile(battle.txt) %pokename $+ $1 $+ _clone

  if ($readini($char(%pokename $+ $1 $+ _clone), info, flag) = npc) {
    if (%battle.type = ai) {
      var %total.players $readini($txtfile(battle2.txt), BattleInfo, Players)
      inc %total.players 1
      writeini $txtfile(battle2.txt) BattleInfo Players %total.players
  ...
  if ($readini($char(%pokename $+ $1 $+ _clone), status, ignition.on) = on) { $revert(%pokename $+ $1 $+ _clone, $readini($char(%pokename $+ $1 $+ _clone), status, ignition.name)) }
  if ($readini($char(%pokename $+ $1 $+ _clone), styles, equipped) = doppelganger) { 
    var %style.chance $rand(1,3)
    if (%style.chance = 1) { writeini $char(%pokename $+ $1 $+ _clone) styles equipped trickster }
    if (%style.chance = 2) { writeini $char(%pokename $+ $1 $+ _clone) styles equipped guardian }
    if (%style.chance = 3) { writeini $char(%pokename $+ $1 $+ _clone) styles equipped weaponmaster }
  }

  writeini $txtfile(battle2.txt) style $1 $+ .lastaction throwpoke

  if (%portal.bonus != true) { writeini $char(%pokename $+ $1 $+ _clone) info FirstTurn true }

  ; Initialize the action points 
  $action.points(%pokename $+ $1 $+ _clone, initialize)

  $skill.nextturn.check(Throwpoke, $1)
}
...
; SNATCH
;=================
on 3:TEXT:!moveto*:*: { $partial.name.match($nick, $2)  | $skill.snatch($nick, %attack.target) }

alias skill.snatch { $set_chr_name($1)
  set %co2 $readini($char($1), Skills, snatch.time)
  if ($person_in_mech($1) = true) { $display.message($readini(translation.dat, errors, Can'tDoThatInMech), private) | halt }
  $no.turn.check($1)
  if (no-skill isin %battleconditions) { $display.message($readini(translation.dat, battle, NotAllowedBattleCondition),private) | halt }
  $amnesia.check($1, skill) 
  ...
  ; Check to see if enough time has elapsed
  $skill.turncheck($1, Snatch, !moveto, false)
  ...
  set %bobject $readini($dbfile(battlefields.db), %current.battlefield, objects)
  if ($isfile($boss($2)) = $true) { $set_chr_name($1) | $display.message($readini(translation.dat, errors, CannotSnatchBosses), private) | halt }
  if (%co2 <= 0) { var %mess 4Sorry $nick but you can only 1 times per battle using !moveto to increase your defense by setting behind an object | $display.message(%mess,private) | halt }
  ;Decrease the action points
  $action.points($1, remove, $skill.actionpointcheck(snatch))

  ; Display the desc. 
  $set_chr_name($2) | set %enemy %real.name
  if ($readini($char($1), descriptions, snatch) = $null) { set %skill.description moves behind  %enemy and tries to use $gender2($2) as a shield and increase its defense! }
...
alias do.snatch {
  ...
  if (%user.level < %target.level) { inc %snatch.chance 5 }
  var %type $readini($char($2), Monster, Type)
  if (%type == object) { 
    writeini $char($1) skills CoverTarget $2
    var %def $readini($char($2), BaseStats, def)
    var %user.def $readini($char($1), battle, def)
    inc %user.def %def
    writeini $char($1) Battle def %user.def
    var %mess 11 $+ %real.name is now behind $2 
  ...
  ; write the last used time.
  writeini $char($1) skills snatch.time 0
