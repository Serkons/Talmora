...
dungeon.start {
  ...
  if (%dungeon.max.players = 0) { $display.message(04This $iif(%portal.bonus = true, portal, dungeon) is level12 %dungeon.level 04and requires12 %dungeon.players.needed $iif(%dungeon.players.needed = 1, player, players) 04to enter (12 $+ %dungeon.players.recommended $+ 04 $iif(%dungeon.players.recommended = 1, player, players) recommended). The $iif(%portal.bonus = true, portal, dungeon) will begin in12 $duration(%time.to.enter) $+ 04. Use !enter if you wish to join the party. , global) }
  else { $display.message(04This $iif(%portal.bonus = true, portal, dungeon) is level12 %dungeon.level 04and will start after12 %dungeon.max.players $iif(%dungeon.max.players = 1, player 04enters, players 04enter) or in12 $duration(%time.to.enter) $+ 04. Use !enter if you wish to join the party. , global) }
...
dungeon.begin {
  ...
  if ((%number.of.players = 0) || (%number.of.players < $readini($txtfile(battle2.txt), DungeonInfo, PlayersNeeded))) { $display.message($readini(translation.dat, errors, NotEnoughPartyMembersForDungeon), global) | remove $dungeonfile(CreatedDungeon) | unset %random.chance1 | $clear_battle | halt }
