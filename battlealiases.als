...
boost_monster_stats {
  ...
  ; Minecraft battles use the final level from the Minecraft system.
  if ((%mc.battle = on) && (%mc.encounter.level != $null)) {
    var %monster.level %mc.encounter.level
  }

  ; Adjust the stats
  $levelsync($1, %monster.level)

  ; Minecraft: equipped weapon level follows encounter level
  if ((%mc.battle = on) && (%mc.encounter.level != $null)) {
    var %mc.weapon $readini($char($1), weapons, equipped)

    if (%mc.weapon != $null) {
      writeini $char($1) weapons %mc.weapon %mc.encounter.level

      ; TEST: Read the abilities belonging to the equipped weapon
      var %mc.abilities $readini($dbfile(weapons.db), %mc.weapon, Abilities)

      echo -a MC WEAPON DEBUG: %mc.weapon
      echo -a MC ABILITIES DEBUG: %mc.abilities

      ; TEST: Split weapon abilities into individual techniques
      var %mc.tech.count $numtok(%mc.abilities,46)
      var %mc.tech.index 1

      echo -a MC TECH COUNT DEBUG: %mc.tech.count

      while (%mc.tech.index <= %mc.tech.count) {
        var %mc.tech $gettok(%mc.abilities,%mc.tech.index,46)

        echo -a MC TECH DEBUG: %mc.tech

        var %mc.tech.level $readini($char($1), Techniques, %mc.tech)

        if (%mc.tech.level != $null) {
          echo -a MC TECH EXISTS DEBUG: %mc.tech = %mc.tech.level
          writeini $char($1) Techniques %mc.tech %mc.encounter.level
        }
        else {
          echo -a MC TECH EXISTS DEBUG: %mc.tech = NOT FOUND
        }
...
boost_monster_hp {
  ...
  if (ignoreHP isin $readini($char($1), info, BattleStats)) { msg %battlechan HP wurde Ignoriert! | return }
...
levelsync {
  ...
  echo -ag LEVELSYNC TEST: $1
  echo -ag TARGET LEVEL: $2
  echo -ag LOOPS: %current.loop
  echo -ag STR: $readini($char($1), battle, str)
  echo -ag DEF: $readini($char($1), battle, def)
  echo -ag INT: $readini($char($1), battle, int)
  echo -ag SPD: $readini($char($1), battle, spd)
  echo -ag LEVEL: $get.level($1)
...
deal_damage {
  ...
  if (%attack.damage > 0) {
    ...
    if ((%naturalArmorCurrent != $null) && (%naturalArmorCurrent > 0)) {
      ...
      if (%naturalArmorCurrent > 0) { $set_chr_name($2) | set -u3 %c.target $2 | $CriticalCut_Check($1) | set %guard.message $readini(translation.dat, battle, NaturalArmorAbsorb) | set %attack.damage 0 }
...
display_damage {
  ...
  if ($3 = weapon) { 
    ...
    if (%counterattack != shield) {
      ...
      if (%counterattack != on ) {   
        set %attacker $1 
        set %target $2 
        var %weapon.type $readini($dbfile(weapons.db), $4, type) 
      ...
      if (Enor isin $2) && (EnorPearl isin $readini($txtfile(battle2.txt), Battle, List)) {
        var %Objectmessage Its an Object and %user does some damage on it.
        var %enor on
      }
    $display.message(03 $+ %user $+  $iif(%enor == on, %objectmessage, $read %attack.file)  $+ 03., battle)  }
  }
  ...
  if ($3 = tech) {
    if (%showed.tech.desc != true) { 
      ...
      $display.message(03 $+ %user $+  $iif(%enor == on, %objectmessage, %tech.desc) , battle)
    }
  ...
  if ($readini($char(%target), battle, HP) <= 0) { 
    ...
    ;;msg #bot-lounge %user defeated %enemy in channel #AiroRPG
    // Orb Reward by Killing and Objekt
    $randoms.redorb.reward
    // End Orb Reward by Killing An Objektz
    unset %overkill
    ...
    if ($readini($char(%target), Stuff, Drops) != $null) && (Core_Stone isin $readini($txtfile(battle2.txt), Battle, List)) {
      set %drop.chance.item 100
      if (%drop.chance.item >= 100) {
...
taunt {
  ...
  if ($readini($char($2), info, HealByTaunt) = true) { 
    ...
    if (%taunt.effect = 1) { var %taunt.str $readini($char($2), battle, str) | inc %taunt.str $rand(1,%taunt.str) | writeini $char($2) battle str %taunt.str | $set_chr_name($2) | $display.message($readini(translation.dat, battle, TauntRage), battle) }
    if (%taunt.effect = 2) { var %taunt.def $readini($char($2), battle, def) | inc %taunt.def $rand(1,%taunt.def) | writeini $char($2) battle def %taunt.def | $set_chr_name($2) | $display.message($readini(translation.dat, battle, TauntDefensive), battle) }
    if (%taunt.effect = 3) { var %taunt.int $readini($char($2), battle, int) | dec %taunt.int $rand(1,%taunt.int) | writeini $char($2) battle int %taunt.int | $set_chr_name($2) | $display.message($readini(translation.dat, battle, TauntClueless), battle) }
    if (%taunt.effect = 4) { var %taunt.str $readini($char($2), battle, str) | dec %taunt.str $rand(1,%taunt.str) | writeini $char($2) battle str %taunt.str | $set_chr_name($2) | $display.message($readini(translation.dat, battle, TauntTakenAback), battle) }
    if (%taunt.effect = 5) { $set_chr_name($2) | writeini $char($2) status bored yes | writeini $char($1) status bored yes | $display.message($readini(translation.dat, battle, TauntBored), battle) }
...
inflict_status {
  ...
  if ($3 = random) { 
    ...
    if (%random.status.type = 21) { set %status.type bleed | var %status.grammar bleeding }
  ...
  if ($3 = burning) { set %status.type burning | var %status.grammar burning }
  if ($3 = silence) { set %status.type silence | var %status.grammar silenced }
  if ($3 = bleed) { set %status.type bleed | var %status.grammar bleeding }
  ...
  if ((%resist.skill < 100) || (%resist.skill = $null)) {
    ...
    if (%chance > 45) { $set_chr_name($2) 
      if (%statusmessage.display != $null) {  set %statusmessage.display %statusmessage.display :: $set_chr_name($2) %real.name receives the status  %status.grammar $+  as a result of the attack! } 
      if (%statusmessage.display = $null) {   $set_chr_name($2) | set %statusmessage.display 04 $+ %real.name receives the status  %status.grammar $+  as a result of the attack! }
      ...
      ;var %m.hp $readini($char($2), BaseStats, hp)
      ;var %c.hp $readini($char($2), battle, hp)
      ;set %percent.hp.current $round($calc(%c.hp / %m.hp * 100),0)
      ;if (%percent.hp.current <= 100) && (%percent.hp.current >= 76) { var %burn.type 3(  10Slight Burn 3 $+ ) | set %burnstatus 10Slight  }
      ;if (%percent.hp.current <= 75) && (%percent.hp.current >= 51) { var %burn.type 3( 7Medium Burn  3 $+ ) | set %burnstatus 7Medium  }
      ;if (%percent.hp.current <= 50) && (%percent.hp.current >= 31) { var %burn.type 3( 7Medium Burn  3 $+ ) | set %burnstatus 7Medium  }
      ;if (%percent.hp.current <= 31) && (%percent.hp.current >= 1) { var %burn.type 3( 4Servere Burn  3 $+ ) | set %burnstatus 4Servere  }

      ;set %status.type burning 
      ;var %status.grammar burning
      ;}
...
do.self.inflict.status {
  ...
  if (%self.status.type = bleed) { set %status.type bleed | var %status.grammar bleeding }
...
winningstreak.addmonster.amount {
  ...
  if ((%pcrystalrun = on) && ($return_winningstreak < 500)) {  set %number.of.monsters.needed $rand(1,6) | set %mon.need %number.of.monsters.needed }
  return
}
...
battlefield.object {
  ...
  while (%n >= 1) {
    ...
    writeini $char(%object.chosen) basestats HP %hpadd
    writeini $char(%object.chosen) basestats def %def
    writeini $char(%object.chosen) battle def %def
    writeini $char(%object.chosen) battle HP %hpadd
    ...
    writeini $char(%object.chosen) Skills Resist-bleed yes
...
battlefield.event {
  ...
  var %random.chance $rand(1,100)
  msg %battlechan Event $+ %battlefield.event.number $+ chance ist bei %random.chance

  if (%random.chance > $readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number $+ chance)) { unset %battlefield.event.number | unset %number.of.events | msg %battlechan ich tue kein Event | return }

  /msg %battlechan Ich Tude das Event!!!

  set %battlefield.event.target $readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number $+ target)

  /msg %battlechan Das Event Ziel ist %Battlefield.event.target
  ...
  if (%battlefield.event.target = all) {
    var %battleevent $readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number)
    var %mess 2====7 Battlefield Event: 7  %battleevent 2====
    $iif(%event.status.type != $null && $readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number) != damage || $readini($dbfile(battlefields.db), %current.battlefield, event $+ %battlefield.event.number) != heal,$display.message(%mess, battle))
...
covercheck {
  ...
  var %type $readini($char(%cover.target), Monster, Type)
  if ((%cover.status = dead) || (%cover.status = runaway) && (%type == $null)) { writeini $char($1) skills CoverTarget none | set %attack.target $1 | return } 
  if ((%cover.status = dead) || (%cover.status = runaway) && (%type == object)) { writeini $char($1) skills CoverTarget none | set %attack.target $1 | writeini $char($1) Battle def $readini($char($1), BaseStats, def) | return } 
  ...
  if (%type == object) {
    var %mess 7 %cover.target takes damage from $2 $+ 's attack.
    $display.message(%mess,battle)
  }
}
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
get.random.reward {
  ...
  if (%reward.list = $null) { return }
  if (Core_Stone isin $readini($txtfile(battle2.txt), Battle, List)) { return }
  var %drop.reward $gettok(%reward.list,$rand(1,$numtok(%reward.list, 46)),46)
...
bleed_check { 
  ...
  if (($readini($char($1), status, bleed) = yes) || ($readini($char($1), status, HeavyBleed) = yes)) {
    ...
    if (%bleed.timer < $status.effects.turns(bleed)) {
      ...
      set %max.hp $readini($char($1), basestats, hp)
      set %bleed $round($calc(%max.hp * .15),0)
      set %hp $readini($char($1), Battle, HP) | unset %max.hp
...
HeavyBleed { 
  ...
  set %max.hp $readini($char($1), basestats, hp)
  set %bleed $round($calc(%max.hp * .20),0)
...
HeavyPoison { 
  ...
  set %max.hp $readini($char($1), basestats, hp)
  set %poison $round($calc(%max.hp * .25),0)
  set %hp $readini($char($1), Battle, HP) | $set_chr_name($1)
  unset %max.hp
...
burning_check { 
  if ($readini($char($1), Status, burning) = yes) {
    ...
    var %randb $rand(1,50)
    if (%randb <= 9) { set %randa .0 $+ %randb }
    if (%randb >= 10) && (%randb <= 39) { set %randa . $+ %randb }
    if (%randb >= 40) && (%randb <= 50) { set %randa . $+ %randb }



    //
    var %m.hp $readini($char($1), BaseStats, hp)
    var %c.hp $reaini($char($1), battle, hp)
    set %burning $round($calc(%max.hp * %randa),0)
    unset %max.hp |  set %hp $readini($char($1), Battle, hp)
    $set_chr_name($1)
    ...
    set %hp.base $readini($char($1), BaseStats, hp)
    set %percent.hp.current2 $round($calc(%hp / %m.hp * 100),0)
    if (%percent.hp.current2 <= 99) && (%percent.hp.current2 >= 75) { var %burn.type 3(  10Slight Burn 3 $+ ) | set %burnstatus 10Slight  }
    if (%percent.hp.current2 <= 75) && (%percent.hp.current2 >= 50) { var %burn.type 3( 7Medium Burn  3 $+ ) | set %burnstatus 7Medium  }
    if (%percent.hp.current2 <= 49) && (%percent.hp.current2 >= 26) { var %burn.type 3( 7Medium Burn  3 $+ ) | set %burnstatus 7Medium  }
    if (%percent.hp.current2 <= 25) && (%percent.hp.current2 >= 1) { var %burn.type 3( 4Servere Burn  3 $+ ) | set %burnstatus 4Servere  }
