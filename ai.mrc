...
alias ai_skillcheck {
  ...
  if ($readini($char($1), skills, deathstrike) != $null) { 
    if ($readini($char($1), skills, deathstrike.on) != on) {
      remini $char($1) skills deathstrike.time | %ai.skilllist  = $addtok(%ai.skilllist, deathstrike, 46) 
    }
  }
  ...
  if ($readini($char($1), skills, aggressor) != $null) { 
    if ($readini($char($1), skills, aggressor.on) != on) {
      writeini $char($1) skills aggressor.time 0 | %ai.skilllist  = $addtok(%ai.skilllist, aggressor, 46) 
    }
  }
  if ($readini($char($1), skills, defender) != $null) { 
    if ($readini($char($1), skills, defender.on) != on) {
      writeini $char($1) skills defender.time 0 | %ai.skilllist  = $addtok(%ai.skilllist, defender, 46) 
    }
  }
...
alias ai_chooseskill {
  ...
  if (%ai.skill = deathstrike) { $skill.deathstrike($1) }
  if (%ai.skill = truestrike) { $skill.truestrike($1) }
  if (%ai.skill = aggressor) { $skill.aggressor($1) }
  if (%ai.skill = defender) { $skill.defender($1) }
