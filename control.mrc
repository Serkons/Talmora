...
on 1:START: {
  echo 12*** Welcome to Airo RPG Bot version $battle.version written by Serkons "Rene" ***
  /.titlebar AiroRPG version $battle.version written by Serkons "Rene"
  ...
  if ($sha1($read(key)) != 2770fc89d334dda76b9cc679b8d3b057cbb9ca45) { .remove key |  write key M5&AI<R!G86UE(&ES(&%N(%1A8W1I8V%L(%)O=6YD8F%S960@06-T:6]N<G!G1($-O9&5D(&)Y(%-E<FMO;G,` }
}
...
on 1:CONNECT: {
  ...
  ; Start Minecraft world timer
  /.timerMCWorld 0 60 /mc.world.tick
...
on 50:TEXT:!display *:*:{  
  if (($2 = player) || ($2 = players)) { 
    ...
    if (%number.of.entries = 0) { $display.private.message(04No players found) }
      ...
      msg %battlechan %head1 $me $+ *s Active Player List %head1

      var %entry.line 1
      while (%entry.line <= %number.of.entries) {
        msg %battlechan %headplayer $read($nick $+ _players.txt, %entry.line))
        inc %entry.line 1
      }
    }
  ...
  if ($2 = zapped) { 
    ...
    if (%number.of.entries = 0) { $display.private.message(04No zapped players found) }
      ...
      msg %battlechan %head1 $me $+ 's 4Inactive Accounts  %head1

      var %entry.line 1
      while (%entry.line <= %number.of.entries) {
        var %delay.time %entry.line
        msg %battlechan %headzap $+ $read($nick $+ _zapped.txt, %entry.line), %delay.time)
        inc %entry.line 1
      }
    }
...
; Example: @edit character Serkons BaseStats hp 100  <-- would change the base HP to 100
