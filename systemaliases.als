...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
quitmsg { return AiroRPG by Serkons }
...
copyini {
  ...
  ; For example: $copyini(Serkons, equipment, gearset1)
...
system.start.newbattle {
  ...
  if ((%special.flag = $null) && ($current.battlestreak >= 1)) {
    var %bonuschance $rand(1,100)
    msg #AiroRPG Bonustruhenchance %bonuschance
    if (%bonuschance == 15) || (%bonuschance == 30) || (%bonuschance == 45) || (%bonuschance == 60) {
      $bonusmat 
    }
  }

  if ((%special.flag = $null) && ($current.battlestreak >= 1)) {
    set %random.chance1 $rand(1,100)
    msg #AiroRPG Dungeonchance liegt bei %random.chance1
    if (%random.chance1 == 25) {
      ...
      while (%x <= %froom) {
        ...
        set %chestchance $rand(1,100)
        set %bossroom1 $readini(%dfolder $+ CreatedDungeon $+ .dungeon, %froom, bossroom)
        echo -a Bosschance: %rand.chance
        echo -a ChestChance %chestchance
        echo -a RestRoomchance %restorechance
        ;;if (%restorechance <= 30) { writeini %dfolder $+ CreatedDungeon $+ .dungeon %x RestoreRoom true | echo -a RestoreRaum Erstellt: Raumnummer %x }
        ...
        if ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, ChestRoom) == $null) && ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, RestoreRoom) == $null) { 
          if (%restorechance >= 85) && ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, RestoreRoom) != $null) { inc %x }
          if (%restorechance >= 85) && ($readini(%dfolder $+ CreatedDungeon $+ .dungeon, %x, RestoreRoom) == $null) { 
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Desc 2[Room: 8RestRoom2] In This room players automatically Healed or so....
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Battlefield %dname
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x RestoreRoom true
            echo -a RestoreRaum Erstellt: Raumnummer des Restoraums %x 
            inc %x
          }
          if (%chestchance <= 10) { 
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Desc 2[Room: 8ChestRoom2] This is a Chestroom the content gets a player
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Battlefield %dname
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x ChestRoom true
            echo -a ChestraumRaum Erstellt: Raumnummer des Chestraumes %x
            inc %chestanzahl  
            inc %x
          }
          if (%restorechance <= 85) || (%chestchance >= 10) {
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Desc 2[Room: 7 %x 2] $read($txtfile(desc.txt))
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Battlefield %dname
            echo -a Monsterraum Erstellt: Raumnummer %x 
            writeini %dfolder $+ CreatedDungeon $+ .dungeon %x Monsters $readini($dungeonfile(CreatedDungeon), MonsterList, $rand(1,11))
          }
        }
        inc %x
      }
      ...
      msg #AiroRPG Anzahl der Truhen %chestanzahl
  ...
  if (%special.flag = boss savethepresident) { 
    $display.message($readini(translation.dat, Battle, StartBattlePresident), global)
...
mcdbfile { return " $+ $mircdir $+ minecraft\db\ $+ $1 $+ " }
mclstfile { return " $+ $mircdir $+ minecraft\lst\ $+ $1 $+ " }
...
display.message {
  ...
  if ($readini(system.dat, system, botType) = Discord) {
    ...
    var %message.to.display $remove(%message.to.display,,$chr(59))
    /msg %battlechan %message.to.display
...
display.message.delay {
  ...
  if ($readini(system.dat, system, botType) = Discord) {
    ...
    var %message.to.display $remove(%message.to.display,,$chr(95))
    /msg %battlechan %message.to.display
    ;;/.timerThrottleDisplayMessage $+ $2 $+ $rand(1,100) $+ $rand(a,z) $+ $rand(1,1000) -d 1 %delay.time /msg %battlechan %message.to.display
...
fulls {  
  ...
  ; Minecraft post-battle HP test
  var %mc.postbattle false

  if ((%mc.battle = on) && (%players.must.die.mode = true) && (%battleis = off)) {
    if ($istok($readini($txtfile(battle2.txt), Battle, List), $1, 46) = $true) {
      var %mc.postbattle true
      var -s %mc.saved.hp $readini($char($1), Battle, HP)
      var -s %mc.saved.status $readini($char($1), Battle, Status)
    }
  }
  ...
  ; Minecraft post-battle handling
  if (%mc.postbattle = true) {

    ; Spieler hat den Kampf überlebt
    if ((%mc.saved.status != dead) && (%mc.saved.hp > 0)) {
      writeini $char($1) Battle HP %mc.saved.hp
      writeini $char($1) Battle Status %mc.saved.status

      echo -ag MC POSTBATTLE: $1 survived with %mc.saved.hp HP.
    }

    ; Spieler ist wirklich gestorben
    else {
      var -s %player $1
      writeini $char($1) Battle HP %mc.saved.hp
      writeini $char($1) Battle Status %mc.saved.status

      echo -ag MC POSTBATTLE: $1 died - starting respawn.

      .timer.respawnmc 1 2 /mc.respawn $1
    }
  }
...
clear_skills {
  ...
  writeini $char($1) skills deathstrike.on off 
...
get_mon_list {
  ...
  ; Minecraft encounter already selected the monster
  if ((%mc.battle = on) && (%mc.encounter.monster != $null)) {
    set %monster.list %mc.encounter.monster
    return
  }
...
hp_status_hpcommand { 
  ...
  if ((%hp.percent < 90) && (%hp.percent >= 80)) { set %hstats $readini(translation.dat, health, good) | return }
  if ((%hp.percent < 80) && (%hp.percent >= 70)) { set %hstats $readini(translation.dat, health, decent) | return }
  if ((%hp.percent < 70) && (%hp.percent >= 60)) { set %hstats $readini(translation.dat, health, scratched)  | return }
  if ((%hp.percent < 60) && (%hp.percent >= 50)) { set %hstats $readini(translation.dat, health, bruised) | return }
  if ((%hp.percent < 50) && (%hp.percent >= 40)) { set %hstats $readini(translation.dat, health, hurt) | return }
  if ((%hp.percent < 40) && (%hp.percent >= 30)) { set %hstats $readini(translation.dat, health, injured) | return }
  if ((%hp.percent < 30) && (%hp.percent >= 15)) { set %hstats $readini(translation.dat, health, injuredbadly) | return } 
...
clear_variables { 
  ...
  unset %hq-gen on | unset %hq-gen.drop.rewards | unset %burnstatus
...
powercrystal.check {
  ...
  ;;;msg %battlechan SpyReportchance is : %spyreport
  if (%spyreport >= 50) && (%crystal.power <= 99) { msg %battlechan %headInfo Event: 12PowerCrystal %headinfo1 | $display.message($readini(translation.dat, System, CheckCrystalPower), global) }
...
fightlevel {
  return 14,1 10Battle Level:14 ( $iif($readini(battlestats.dat, Battle, LosingStreak) != 0, -) $+ 08 $+ $current.battlestreak  14)
}

commands {

  msg %battlechan %headcom 10Command: 7!enter10 = 14joins the battle
  msg %battlechan %headcom 10Command: 7!go10 = 14Starts the battle
