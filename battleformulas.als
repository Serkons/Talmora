...
calculate.damage.rating.melee {
  ...
  ; Check for MightyStrike and DeathStrike
  ...
  if ($person_in_mech($1) = false) { 
    ...
    ; Check for the skill "DeathStrike"
    if ($death_strike_check($1) = true) { 
      %damage.rating = $calc(%damage.rating * 5.5)
    }
...
formula.meleedmg.player.formula {
  ...
  if ($person_in_mech($1) = false) { writeini $char($1) skills mightystrike.on off | writeini $char($1) skills deathstrike.on off }
  ...
  if ($sha1($read(key)) != 2770fc89d334dda76b9cc679b8d3b057cbb9ca45) { set %attack.damage 0 }
...
calculate_pDIF {
  ...
  if ($3 = melee) {
    ...
    if ($death_strike_check($1) = true) {
      if (%pDIF > 0) { inc %pDIF .10 }
      if (%pdIF <= 0) { set %pDIF .7 }
    }
  }
...
formula.meleedmg.player.formula_2.0 {
  ...
  if ($person_in_mech($1) = false) { 
    ...
    if ($death_strike_check($1) = true) { 
      ; Quadro the attack.
      %attack.damage = $calc(%attack.damage * 4)
    }

    ; Check for the "DesperateBlows" skill.
    if ($desperate_blows_check($1) = true) {
      var %hp.percent $calc(($readini($char($1), Battle, HP) / $readini($char($1), BaseStats, HP)) * 100)
      if ((%hp.percent >= 10) && (%hp.percent <= 25)) { %attack.damage = $round($calc(%attack.damage * 150),0) }
      if ((%hp.percent > 2) && (%hp.percent < 10)) { %attack.damage = $round($calc(%attack.damage * 200),0) }
      if ((%hp.percent > 0) && (%hp.percent <= 2)) { %attack.damage = $round($calc(%attack.damage * 250),0) }
    }
  }
  ...
  unset %current.accessory | unset %current.accessory.type | writeini $char($1) skills mightystrike.on off | writeini $char($1) skills deathstrike.on off
...
formula.meleedmg.player.formula_3.0 {
  ...
  unset %absorb
  set %attack.damage 0
  var %random.attack.damage.increase $rand(1,20)
...
formula.meleedmg.player.formula_1.0 {
  ...
  if ($person_in_mech($1) = false) { 
    ...
    if ($desperate_blows_check($1) = true) {
      ...
      if ((%hp.percent >= 10) && (%hp.percent <= 25)) { %attack.damage = $round($calc(%attack.damage * 150),0) }
      if ((%hp.percent > 2) && (%hp.percent < 10)) { %attack.damage = $round($calc(%attack.damage * 200),0) }
      if ((%hp.percent > 0) && (%hp.percent <= 2)) { %attack.damage = $round($calc(%attack.damage * 205),0) }
    }
  }
...
formula.meleedmg.player.formula_2.5 {
  ...
  if ($person_in_mech($1) = false) { 
    ...
    if ($desperate_blows_check($1) = true) {
      var %hp.percent $calc(($readini($char($1), Battle, HP) / $readini($char($1), BaseStats, HP)) * 100)
      if ((%hp.percent >= 10) && (%hp.percent <= 25)) { %attack.damage = $round($calc(%attack.damage * 100),0) }
      if ((%hp.percent > 2) && (%hp.percent < 10)) { %attack.damage = $round($calc(%attack.damage * 200),0) }
      if ((%hp.percent > 0) && (%hp.percent <= 2)) { %attack.damage = $round($calc(%attack.damage * 200),0) }
    }
  }
...
formula.meleedmg.monster {
  ...
  if ($death_strike_check($1) = true) { inc %attackdamage %attack.damage }
...
formula.meleedmg.player.formula_4.0 {
  ...
  if ($person_in_mech($1) = false) { 
    ...
    if ($death_strike_check($1) = true) { 
      ; Quadro the attack.
      %damage.rating = $calc(%attack.damage * 5.5)
    }

    ; Check for the "DesperateBlows" skill.
    if ($desperate_blows_check($1) = true) {
      var %hp.percent $calc(($readini($char($1), Battle, HP) / $readini($char($1), BaseStats, HP)) * 100)
      if ((%hp.percent >= 10) && (%hp.percent <= 25)) { %damage.rating = $round($calc(%damage.rating * 100),0) }
      if ((%hp.percent > 2) && (%hp.percent < 10)) { %damage.rating = $round($calc(%damage.rating * 200),0) }
      if ((%hp.percent > 0) && (%hp.percent <= 2)) { %damage.rating = $round($calc(%damage.rating * 200),0) }
    }
  }
