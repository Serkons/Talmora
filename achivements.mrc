...
alias achievement_check {
  ...
  if ($2 = FirstKill) {
    var %monsterkills $readini($char($1), Stuff, MonsterKills)
    var %tutorial $readini($char($1), Info, Tutorial)
    if (%monsterkills = 1) && (%tutorial = on) { writeini $char($1) achievements $2 true
      $announce_achievement($1, $2, 1)
      writeini $char($1) Item_Amount Present 1
      remini $char($1) Info Tutorial
    }
  }
