...
check_monster {
  ...
  while (%i < %count) {
    ...
    if (%skill isin CocoonEvolve.MonsterConsume.Snatch.MagicShift.DemonPortal.MonsterSummon.RepairNaturalArmor.ChangeBattlefield.Quicksilver) noop
      ...
      if (%status_resisted !isin stop.poison.silence.blind.drunk.virus.amnesia.paralysis.bleed.zombie.slow.stun.curse.charm.intimidate.defensedown.strengthdown.intdown.petrify.bored.confuse.sleep) $&
  ...
  if ($readini(%file_path, skills, deathstrike.on)) if ($readini(%file_path, skills, deathstrike.on) !isin on.off) log_issue Minor %category $+  $1  has an initial deathstrike.on field, but it isn't 'on'; it will be ignored.
  ...
  if ($1 != $null) { 
    ...
    if (%count >= 1) {
      ...
      while (%i <= %count) { 
        ...
        else if (%current.status.effect !isin stop.poison.burning.silence.blind.drunk.virus.amnesia.paralysis.zombie.slow.stun.curse.charm.intimidate.defensedown.strengthdown.intdown.petrify.bored.confuse.defenseup.speedup.sleep.random) $&
