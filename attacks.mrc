...
alias attack_cmd { 
  ...
  if ($1 == Enor) { $display.message(Its a Pearl,battle) | /halt }
...
alias death_strike_check {
  var %deathstrike $readini($char($1), skills, deathstrike.on)
  if (%deathstrike = on) { return true }
  else { return false }
}
...
alias CriticalCut_check {
  ...
  if ($readini($char($1), skills, CriticalCut) > 0) {
    ...
    ;;msg #AiroRPG Bleed-Chance: %bleed.chance CritCutChance: %CriticalCut.chance

    if ($augment.check($1, EnhanceCriticalCut) = true) { inc %CriticalCut.chance %augment.strength  } 

    if (%bleed.chance == 21)  && (%CriticalCut.chance == 20) {
      var %natura $readini($char(%c.target), NaturalArmor, Current)
      if (%natura isnum) && (%natura >= 0) { var %natural-armor yes }
      set %resist-bleed1 $readini($char($2), Skills, Resist-bleed)
      set %resist-bleed2 $readini($char(%c.target), Skills, Resist-bleed)
      if (%resist-bleed1 != $null) {
        $set_chr_name($2) | set %statusmessage.display 04  $1 could not bleed %real.name .
      }
      if (%c.target != $null) && (%resist-bleed2 == $null) && (%natural-armor == yes) {
        $set_chr_name(%c.target) 
        set %statusmessage.display 04  $1 could not bleed %real.name .
      }  
    }
    if (%bleed.chance <= %CriticalCut.chance) {
      ...
      if (%resist-bleed1 == $null) && (%natural-armor == $null) {
        $set_chr_name($2) | writeini $char($2) status bleed yes |  set %statusmessage.display 04 $+ %real.name suffered a deep cut and is now bleeding to death!
      }
      if (%c.target != $null) && (%resist-bleed2 == $null) && (%natural-armor == yes) {
        $set_chr_name(%c.target) | writeini $char(%c.target) status bleed yes | /msg #AiroRPG 4 The Attack goes through the Armor causing bleeding | set %statusmessage.display 04 $+ %real.name suffered a deep cut and is now bleeding to death!
      }  
    }
    if (%resist-bleed == 100) { return }
  }
}
