...
on 50:TEXT:!start bat*:*:{   
  if (%battleis = on) { $display.message($readini(translation.dat, errors, BattleAlreadyStarted), private) | halt }
  /.timerBattleStart off | $startnormal($3, $4) | /timer.checkcount1 0 1 /checkcount
}
on 50:TEXT:!new bat*:*:{   
  if (%battleis = on) { $display.message($readini(translation.dat, errors, BattleAlreadyStarted), private) | halt }
...
on 50:TEXT:!end bat*:*:{ $endbattle($3) | unset %player.list | msg %battlechan hi } 
on 50:TEXT:!endbat*:*:{  $endbattle($2)  | unset %player.list } 
...
alias startnormal { 
  ...
  if (%start.battle.type = $null) { 
    ...
    if ($istok(%boss.battle.numbers,$return_winningstreak, 46) = $false) {   
      ...
      if (($return_winningstreak >= 50) && ($return_winningstreak < 100)) { var %valid.battle.types monster.boss.monster.monster.monster.orbfountain.monster.monster }
      if ($return_winningstreak >= 100) {  var %valid.battle.types monster.boss.monster.monster.orbfountain.monster.monster.monster.monster.monster.monster }
  ...
  if (%start.battle.type != ai) {
    ...
    if ((((((%start.battle.type = boss) && ($2 != supplyrun) && ($2 != hq-gen) && ($2 != pcrystalrun) && ($2 != besieged) && ($2 != savethepresident)))))) { 
      ...
      if ($return.systemsetting(showCustomBattleMessages) != true) { msg %battlechan %headInfo Information %headinfo1 | $display.message($readini(translation.dat, Battle, BattleOpen), global)  }
    ...
    if (%start.battle.type = monster) {

      set %battle.type monster

      ; Minecraft Battle
      if (%mc.battle = on) {
        echo -a MC STARTNORMAL DEBUG: Minecraft Battle erkannt!
        var -s %mc.start.biome $readini($char(%mc.encounter.player), Minecraft, Biome)

        var %mc.open.list $readini(translation.dat, Battle, MinecraftBattleOpen)
        var %mc.init.message $read($mclstfile(%mc.open.list))

        $display.message(%mc.init.message, global)

        msg %battlechan 10Minecraft Survival14 - Gegner:04 %mc.encounter.monster 14- Level:07 %mc.encounter.level 14- Biome:03 %mc.start.biome
      }

      ; Normaler Core Battle
      else {
        set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1]
        msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype $fightlevel
        $display.message($readini(translation.dat, Battle, BattleOpen), global)
        $commands
      }
    }
    if ((%start.battle.type = orbfountain) || (%start.battle.type = orb_fountain)) { set %battle.type orbfountain 
      if ($return.systemsetting(showCustomBattleMessages) != true) { set %headfighttype 14,1[9→4 $+ %start.battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype $fightlevel | $display.message($readini(translation.dat, Battle, BattleOpen), global) | $commands  }
      if ($return.systemsetting(showCustomBattleMessages) = true) { set %headfighttype 14,1[9→4 $+ %start.battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype $fightlevel | $display.message($readini(translation.dat, Battle, BattleOpenOrbFountain), global) | $commands }
    }
    ...
    if (%start.battle.type = gauntlet) { set %battle.type monster | set %mode.gauntlet on | set %nosouls true | set %mode.gauntlet.wave 1 | set %headfighttype 14,1[9→4 $+ %start.battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | $display.message($readini(translation.dat, Battle, BattleOpenGauntlet), global)  }
    if (%start.battle.type = manual) { set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | set %battle.type manual | $display.message($readini(translation.dat, Battle, BattleOpenManual), global)  }
    if (%start.battle.type = mimic) { set %battle.type mimic | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | $display.message($readini(translation.dat, Battle, BattleOpenMimic), global) }
    if (%start.battle.type = defendoutpost) { set %battle.type defendoutpost | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype |
      $display.message($readini(translation.dat, Battle, BattleDefendOutpost), global) 
    }
    if (%start.battle.type = assault) { set %battle.type assault | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | $display.message($readini(translation.dat, Battle, BattleAssaultOpen), global) }
    if (%start.battle.type = DragonHunt) { 
      set %battle.type DragonHunt | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | $display.message($readini(translation.dat, system, DragonHunt.BeginHunt), global) 
    ...
    if (%start.battle.type = torment) { set %battle.type torment | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | $display.message($readini(translation.dat, system, TormentBattleStarted), global) }
    if (%start.battle.type = cosmic) { set %battle.type cosmic | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype | $display.message($readini(translation.dat, system, CosmicBattleStarted), global) }
    if (%start.battle.type = monsterfair) { set %battle.type monsterfair | set %headfighttype 14,1[9→4 $+ %battle.type $+ 9←14,1] | msg %battlechan %headInfo Information %headinfo1 - BattleType: %headfighttype |  $display.message($readini(translation.dat, system, MonsterFairBattleStarted), global) }

    var %valid.battle.types ai.boss.monster.orbbattle.orbfountain.orb_fountain.pvp.gauntlet.manual.mimic.defendoutpost.assault.dragonhunt.torment.pcrystalrun.cosmic.hq-gen.besieged.supplyrun.monsterfair.savethepresident
...
on 3:TEXT:!enter:#: {
  ...
  if (%tut == on) && (%current.hp <= 200) && ($current.wp != fists) { msg $chan Sorry $nick it is a risk with no weapon and less than 200 hp to battle. Please skill your char first | halt }
    ...
    set %player.list $addtok(%playerlist,$nick,46)
    $enter($nick)
  }
}
...
alias battlebegin {
  ...
  if (%battle.type != ai) { 
    if ((%number.of.players = 0) || (%number.of.players = $null)) {  
      ...
      msg %battlechan %headInfo Information %headinfo1 - Battle Ends - Next battle in 14(8 $duration(%newbattle.time) 14)
  ...
  if ((%battle.type != ai) && (%battle.type != dragonhunt)) {
    ...
    ;; Make to 25% to generate a Obsidian core.
    var -s %corechance $rand(1,100)
    msg %battlechan Corestonechance liegt bei %corechance
    if (%corechance == 25) {

      .copy -o $mon(core_stone) $char(core_stone)  | set %curbat $readini($txtfile(battle2.txt), Battle, List) | %curbat = $addtok(%curbat,core_stone,46) | writeini $txtfile(battle2.txt) Battle List %curbat | write $txtfile(battle.txt) core_stone
      $boost_monster_stats(core_stone) | $fulls(core_stone)
...
alias endbattle {
  ...
  if ($exists($dungeonfile(CreatedDungeon)) == $true) { remove $dungeonfile(CreatedDungeon) }
...
alias battle.end.failure {
  ...
  if (%savethepresident = on) { $display.message($readini(translation.dat, battle, EvilHasWonPresident), global) 
    ...
    ;;$shopnpc.add(alliedforcepresident)
  }
...
alias battle.end.victory {
  ...
  if (%battle.type = monster) || (%battle.type = boss) && ($current.battlestreak >= 15) {
...
alias battlelist { 
  ...
  if ($1 = public) { 
    ...
    if (Core_Stone isin $readini($txtFile(battle2.txt),Battle,List)) { 
      $display.message($readini(translation.dat,Battle,BattleOpenCoreStone),battle)
      $display.message(4ATTENTION14 An Obsidian Core Chunk is here. You must defeat them to get the core ,battle) 
    }
